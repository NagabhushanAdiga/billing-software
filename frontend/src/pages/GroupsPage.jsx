...
function groupGradient(name = '') {
  ...
  return ICON_GRADIENTS[code % ICON_GRADIENTS.length]
}

function GroupRowIcon({ name }) {
  return (
    <div
      className={`w-11 h-11 rounded-xl bg-gradient-to-br ${groupGradient(name)} flex items-center justify-center text-white shadow-md shrink-0`}
    >
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-8">
      <PageHeader
        ...
        title="Groups"
        description="Create groups to organize products — assigning a group when adding products is optional."
      >
      ...
      <Card className="p-5 sm:p-6">
        ...
        {filtered.length === 0 ? (
          <div className="rounded-xl border-2 border-dashed border-violet-200 py-16 px-4 text-center bg-violet-50/30">
            <div className="inline-flex w-14 h-14 rounded-2xl bg-violet-50 items-center justify-center mb-3">
              <HiOutlineCollection className="w-7 h-7 text-violet-500" />
            </div>
            <p className="text-slate-500 text-sm font-medium">
              {groups.length === 0 ? 'No groups yet. Add your first group.' : 'No groups match your search.'}
            </p>
          </div>
        ) : (
          <div className="rounded-xl border-2 border-violet-100 overflow-x-auto shadow-sm">
                ...
                {filtered.map((g, i) => (
                  <tr key={g.id} className="hover:bg-violet-50/50 transition-colors">
                    <td className="px-4 py-3.5">
                      <GroupRowIcon name={g.name} />
                    </td>
                    <td className="px-4 py-3.5">
                      <p className="text-slate-900 font-semibold text-sm">{g.name}</p>
                      <p className="text-slate-400 text-xs font-mono mt-0.5">{g.id}</p>
                    </td>
                    <td className="px-4 py-3.5">
